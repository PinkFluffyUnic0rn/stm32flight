...
static int pconf_adc_enable_clock(ADC_TypeDef *inst)
{
	...
	else if (inst == ADC2)		__HAL_RCC_ADC2_CLK_ENABLE();
...
{
	if (inst == ADC1)		__HAL_RCC_ADC1_CLK_DISABLE();
	else if (inst == ADC2)		__HAL_RCC_ADC2_CLK_DISABLE();
...
static int pconf_tim_enable_clock(TIM_TypeDef *inst)
{
	...
	else if (inst == TIM9)		__HAL_RCC_TIM9_CLK_ENABLE();
	else if (inst == TIM10)		__HAL_RCC_TIM10_CLK_ENABLE();
	else if (inst == TIM11)		__HAL_RCC_TIM11_CLK_ENABLE();
...
static int pconf_tim_disable_clock(TIM_TypeDef *inst)
{
	...
	else if (inst == TIM9)		__HAL_RCC_TIM9_CLK_DISABLE();
	else if (inst == TIM10)		__HAL_RCC_TIM10_CLK_DISABLE();
	else if (inst == TIM11)		__HAL_RCC_TIM11_CLK_DISABLE();
...
static int pconf_uart_pinalternate(USART_TypeDef *inst)
{
	if (inst == USART1)		return GPIO_AF7_USART1;
	else if (inst == USART2)	return GPIO_AF7_USART2;
	else if (inst == USART3)	return GPIO_AF7_USART3;
	else if (inst == UART4)		return GPIO_AF8_UART4;
	else if (inst == UART5)		return GPIO_AF8_UART5;
	else if (inst == USART6)	return GPIO_AF8_USART6;

	return (-1);
}

static int pconf_i2c_pinalternate(I2C_TypeDef *inst)
{
...
static int pconf_spi_pinalternate(SPI_TypeDef *inst)
{
...
static int pconf_tim_pinalternate(TIM_TypeDef *inst)
{
	...
	else if (inst == TIM9)		return GPIO_AF3_TIM9;
	else if (inst == TIM10)		return GPIO_AF3_TIM10;
	else if (inst == TIM11)		return GPIO_AF3_TIM11;
...
static int pconf_dmastream_uartrx_channel(DMA_Stream_TypeDef *inst,
	USART_TypeDef *uinst)
{
	if (inst == DMA1_Stream0) {
		if (uinst == UART5)		return DMA_CHANNEL_4;
	}
	if (inst == DMA1_Stream1) {
		if (uinst == USART3)		return DMA_CHANNEL_4;
	}
	if (inst == DMA1_Stream2) {
		if (uinst == UART4)		return DMA_CHANNEL_4;
	}
	if (inst == DMA1_Stream5) {
		if (uinst == USART2)		return DMA_CHANNEL_4;
	}
	if (inst == DMA2_Stream1) {
		if (uinst == USART6)		return DMA_CHANNEL_5;
	}
	if (inst == DMA2_Stream2) {
		if (uinst == USART1)		return DMA_CHANNEL_4;
		else if (uinst == USART6)	return DMA_CHANNEL_5;
	}
...
static int pconf_dmastream_uarttx_channel(DMA_Stream_TypeDef *inst,
	USART_TypeDef *uinst)
{
	if (inst == DMA1_Stream3) {
		if (uinst == USART3)		return DMA_CHANNEL_4;
	}
	if (inst == DMA1_Stream4) {
		if (uinst == UART4)		return DMA_CHANNEL_4;
		else if (uinst == USART3)	return DMA_CHANNEL_7;
	}
	if (inst == DMA1_Stream6) {
		if (uinst == USART2)		return DMA_CHANNEL_4;
	}
	if (inst == DMA1_Stream7) {
		if (uinst == UART5)		return DMA_CHANNEL_4;
	}
	if (inst == DMA2_Stream6) {
		if (uinst == USART6)		return DMA_CHANNEL_5;
	}
	if (inst == DMA2_Stream7) {
		if (uinst == USART1)		return DMA_CHANNEL_4;
		else if (uinst == USART6)	return DMA_CHANNEL_5;
	}
...
static int pconf_dmastream_adc_channel(DMA_Stream_TypeDef *inst,
	ADC_TypeDef *adcinst)
{
	if (inst == DMA2_Stream0) {
		if (adcinst == ADC1)		return DMA_CHANNEL_0;
		else if (adcinst == ADC3)	return DMA_CHANNEL_2;
	}
	if (inst == DMA2_Stream1) {
		if (adcinst == ADC3)		return DMA_CHANNEL_2;
	}
	if (inst == DMA2_Stream2) {
		if (adcinst == ADC2)		return DMA_CHANNEL_1;
	}
	if (inst == DMA2_Stream3) {
		if (adcinst == ADC2)		return DMA_CHANNEL_1;
	}
	if (inst == DMA2_Stream4) {
		if (adcinst == ADC1)		return DMA_CHANNEL_0;
	}
	...
	I2C_TypeDef *i2cinst)
{
	...
	if (inst == DMA1_Stream2) {
		if (i2cinst == I2C3)		return DMA_CHANNEL_3;
		else if (i2cinst == I2C2)	return DMA_CHANNEL_7;
	}
	if (inst == DMA1_Stream3) {
		if (i2cinst == I2C2)		return DMA_CHANNEL_7;
	}
	if (inst == DMA1_Stream5) {
		if (i2cinst == I2C1)		return DMA_CHANNEL_1;
	}
...
static int pconf_dmastream_timup_channel(DMA_Stream_TypeDef *inst,
	TIM_TypeDef *timinst)
{
	if (inst == DMA1_Stream0) {
		if (timinst == TIM5)
	...
	else if (inst == DMA1_Stream1) {
		if (timinst == TIM2)
			return DMA_CHANNEL_3;
	}
	else if (inst == DMA1_Stream2) {
		if (timinst == TIM7)
			return DMA_CHANNEL_1;
		else if (timinst == TIM3)
			return DMA_CHANNEL_5;
	}
	else if (inst == DMA1_Stream4) {
		if (timinst == TIM7)
			return DMA_CHANNEL_1;
	}
	else if (inst == DMA1_Stream6) {
		if (timinst == TIM4)
			return DMA_CHANNEL_2;
		else if (timinst == TIM5)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream7) {
		if (timinst == TIM2)
			return DMA_CHANNEL_3;
	}
	else if (inst == DMA2_Stream1) {
		if (timinst == TIM8)
			return DMA_CHANNEL_7;
	}
	else if (inst == DMA2_Stream5) {
		if (timinst == TIM1)
			return DMA_CHANNEL_6;
	}
	...
	TIM_TypeDef *timinst, int ch)
{
	...
	if (inst == DMA1_Stream0) {
		if (timinst == TIM4 && ch == 1)
			return DMA_CHANNEL_2;
		else if (timinst == TIM5 && ch == 3)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream1) {
		if (timinst == TIM2 && ch == 3)
			return DMA_CHANNEL_3;
		else if (timinst == TIM5 && ch == 4)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream2) {
		if (timinst == TIM3 && ch == 4)
			return DMA_CHANNEL_5;
		else if (timinst == TIM5 && ch == 1)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream3) {
		if (timinst == TIM2 && ch == 2)
			return DMA_CHANNEL_2;
		else if (timinst == TIM5 && ch == 4)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream4) {
		if (timinst == TIM3 && ch == 1)
			return DMA_CHANNEL_5;
		else if (timinst == TIM5 && ch == 2)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA1_Stream5) {
		if (timinst == TIM2 && ch == 1)
			return DMA_CHANNEL_3;
		else if (timinst == TIM3 && ch == 2)
			return DMA_CHANNEL_5;
	}
	else if (inst == DMA1_Stream6) {
		if (timinst == TIM2 && (ch == 2 || ch == 4))
			return DMA_CHANNEL_3;
	}
	else if (inst == DMA1_Stream7) {
		if (timinst == TIM4 && ch == 3)
			return DMA_CHANNEL_2;
		else if (timinst == TIM2 && ch == 4)
			return DMA_CHANNEL_3;
		else if (timinst == TIM3 && ch == 3)
			return DMA_CHANNEL_5;
	}
	else if (inst == DMA2_Stream1) {
		if (timinst == TIM1 && ch == 1)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA2_Stream2) {
		if (timinst == TIM8 && (ch == 1 || ch == 2 || ch == 3))
			return DMA_CHANNEL_0;
		else if (timinst == TIM1 && ch == 2)
			return DMA_CHANNEL_6;
		else if (timinst == TIM8 && ch == 1)
			return DMA_CHANNEL_7;
	}
	else if (inst == DMA2_Stream3) {
		if (timinst == TIM1 && ch == 1)
			return DMA_CHANNEL_6;
		else if (timinst == TIM8 && ch == 2)
			return DMA_CHANNEL_7;
	}
	else if (inst == DMA2_Stream4) {
		if (timinst == TIM1 && ch == 4)
			return DMA_CHANNEL_6;
		else if (timinst == TIM8 && ch == 3)
			return DMA_CHANNEL_7;
	}
	else if (inst == DMA2_Stream6) {
		if (timinst == TIM1 && ch == 1)
			return DMA_CHANNEL_0;
		else if (timinst == TIM1 && ch == 3)
			return DMA_CHANNEL_6;
	}
	else if (inst == DMA2_Stream7) {
		if (timinst == TIM8 && ch == 4)
			return DMA_CHANNEL_7;
	}
	...
	DMA_Stream_TypeDef *dmainst)
{
	...
	if (inst == TIM1) {
		if (ch == 1) {
			...
			dmaids[1] = TIM_DMA_ID_TRIGGER;
			dmaids[2] = TIM_DMA_ID_COMMUTATION;
			dmaids[3] = -1;
		}
	}
	else if (inst == TIM2) {
		if (ch == 1) {
			...
			dmaids[0] = TIM_DMA_ID_CC2;
			dmaids[1] = TIM_DMA_ID_CC4;
			dmaids[2] = -1;
		}
		else if (ch == 3) {
			dmaids[0] = TIM_DMA_ID_CC3;
			dmaids[1] = TIM_DMA_ID_UPDATE;
			dmaids[2] = -1;
		}
		else if (ch == 4) {
			dmaids[0] = TIM_DMA_ID_CC2;
...
void pconf_mspinit_uart(UART_HandleTypeDef* huart)
{
	...
	if (uart->usage == PCONF_UARTUSAGE_IRC) {
		...
		GPIO_InitStruct.Alternate = pconf_uart_pinalternate(uart->inst);
...
void pconf_mspinit_i2c(I2C_HandleTypeDef* hi2c)
{
	...
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_InitStruct.Alternate = pconf_i2c_pinalternate(i2c->inst);
...
static void pconf_init_tim_pwm(int idx)
{
	...
	for (i = 0; i < tim->chcnt; ++i) {
		...
		GPIO_InitStruct.Pull = GPIO_NOPULL;
		GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
		GPIO_InitStruct.Alternate = pconf_tim_pinalternate(tim->inst);
...
static void pconf_init_i2c()
{
	...
	for (i = 0; i < PCONF_I2CSCOUNT; ++i) {
		...
		pconf_hi2cs[i].Init.ClockSpeed = 400000;
		pconf_hi2cs[i].Init.DutyCycle = I2C_DUTYCYCLE_2;
